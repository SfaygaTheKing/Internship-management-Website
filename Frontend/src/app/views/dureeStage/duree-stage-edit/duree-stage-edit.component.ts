...
@Component({
  ...
  templateUrl: './duree-stage-edit.component.html',
  styleUrl: './duree-stage-edit.component.css',
})
export class DureeStageEditComponent implements OnInit {
  dateDebut!: Date;
  dateFin!: Date;
  ...
  ngOnInit() {
    ...
    this.dateDebut = new Date(this.selectedDureeStage.debut);
    this.dateFin = new Date(this.selectedDureeStage.fin);
  }

  update(): void {
    if (this.validateForm()) {
      this.selectedDureeStage.debut = this.dateDebut;
      this.selectedDureeStage.fin = this.dateFin;
      this.dureeStageService.update().subscribe({
        next: (data) => {
          if (data !== null) {
            this.messageService.add({
              severity: 'success',
              summary: 'Succès',
              detail: 'Durée de stage mis à jour',
              life: 3000,
            });
            this.returnToList();
          } else {
